...
function getEventParams(event) {
  const params = event.params === void 0 || event.params.length === 0 ? '' : Object.keys(event.params).join(', ')
...
const NAME_PROP_COLOR = ['orange-8', 'accent', 'secondary']
...
function getDiv(col, propName, propValue, slot) {
  return h('div', { class: `api-row__item col-xs-12 col-sm-${col}` }, [
    ...
    propValue !== void 0 ? h('div', { class: 'api-row__value' }, propValue) : slot,
  ])
}
...
function getProp(prop, propName, level, onlyChildren) {
  ...
  if (propName !== void 0) {
    child.push(getExtendedNameDiv(propName, level, type, type !== 'Function' && prop.required === true))
  ...
  if (prop.addedIn !== void 0) {
    child.push(getDiv(12, 'Added in', prop.addedIn))
  }

  child.push(getDiv(12, 'Description', prop.desc))

  if (type === 'Function') {
    child.push(getDiv(12, 'Function form', getMethodParams(prop, true) + getMethodReturnValue(prop)))
  }

  if (prop.sync === true) {
    child.push(getDiv(3, 'Note', 'Required to be used with v-model!'))
  }

  if (prop.default !== void 0) {
    child.push(getDiv(3, 'Default value', JSON.stringify(prop.default)))
  }

  if (prop.link === true) {
    child.push(getDiv(6, 'External link', prop.link))
  }
  ...
  if (prop.definition !== void 0) {
    ...
    for (const propName in prop.definition) {
      nodes.push(getProp(prop.definition[propName], propName, 2))
    }

    child.push(getDiv(12, 'Props', void 0, h('div', { class: 'api-row__subitem' }, nodes)))
  }
  ...
  if (prop.params !== void 0 && prop.params !== null) {
    ...
    for (const propName in prop.params) {
      nodes.push(getProp(prop.params[propName], propName, newLevel))
    }

    child.push(getDiv(12, 'Params', void 0, h('div', { class: 'api-row__subitem' }, nodes)))
  }
  ...
  if (prop.returns !== void 0 && prop.returns !== null) {
    child.push(
      getDiv(
        ...
        void 0,
        h('div', { class: 'api-row__subitem' }, [getProp(prop.returns, void 0, 0)])
      )
    )
  }
  ...
  if (prop.scope !== void 0) {
    ...
    for (const propName in prop.scope) {
      nodes.push(getProp(prop.scope[propName], propName, 1))
    }

    child.push(getDiv(12, 'Scope', void 0, h('div', { class: 'api-row__subitem' }, nodes)))
  }
  ...
  return onlyChildren !== true ? h('div', { class: 'api-row row' }, child) : child
}
...
describe.props = (props) => {
  ...
  for (const propName in props) {
    child.push(getProp(props[propName], propName, 0))
  }
...
describe.slots = (slots) => {
  ...
  for (const slot in slots) {
    child.push(getProp(slots[slot], slot, 0))
  }
  ...
  for (const eventName in events) {
    ...
    if (event.params !== void 0) {
      for (const paramName in event.params) {
        params.push(getProp(event.params[paramName], paramName, 1))
      }
    } else {
      params.push(h('div', { class: 'text-italic q-py-xs q-px-md' }, '*None*'))
    }

    child.push(
      h('div', { class: 'api-row row' }, [
        getNameDiv(`@${eventName}${getEventParams(event)}`, 0),
        event.addedIn !== void 0 ? getDiv(12, 'Added in', event.addedIn) : null,
        getDiv(12, 'Description', event.desc),
        getDiv(12, 'Parameters', void 0, h('div', { class: 'api-row__subitem' }, params)),
      ])
    )
  }
...
describe.methods = (methods) => {
  ...
  for (const methodName in methods) {
    ...
    const nodes = [
      ...
      method.addedIn !== void 0 ? getDiv(12, 'Added in', method.addedIn) : null,
    ...
    if (method.params !== void 0) {
      ...
      for (const paramName in method.params) {
        props.push(getProp(method.params[paramName], paramName, 1))
      }
      nodes.push(getDiv(12, 'Parameters', void 0, h('div', { class: 'api-row__subitem' }, props)))
    }
    if (method.returns !== void 0) {
      nodes.push(
        getDiv(
          ...
          void 0,
          h('div', { class: 'api-row__subitem' }, [getProp(method.returns, void 0, 0)])
        )
      )
    }

    child.push(h('div', { class: 'api-row row' }, nodes))
  }
...
describe.value = (value) => {
  return [
    h(
      'div',
      { class: 'api-row row' },
      [getDiv(12, 'Type', getStringType(value.type))].concat(getProp(value, void 0, 0, true))
    ),
  ]
}
...
describe.arg = (arg) => {
  return [
    h(
      'div',
      { class: 'api-row row' },
      [getDiv(12, 'Type', getStringType(arg.type))].concat(getProp(arg, void 0, 0, true))
    ),
  ]
}
...
describe.modifiers = (modifiers) => {
  ...
  for (const modifierName in modifiers) {
    ...
    child.push(h('div', { class: 'api-row row' }, getProp(modifier, modifierName, 0, true)))
  }
...
describe.injection = (injection) => {
  return [h('div', { class: 'api-row row' }, [getNameDiv(injection, 0)])]
}
...
describe.quasarConfOptions = (conf) => {
  ...
  for (const def in conf.definition) {
    child.push(getProp(conf.definition[def], def, 0))
  }

  conf.addedIn !== void 0 && entry.push(getDiv(12, 'Added in', conf.addedIn))

  entry.push(getDiv(12, 'Definition', void 0, h('div', { class: 'api-row__subitem' }, child)))

  return [h('div', { class: 'api-row row' }, entry)]
}
